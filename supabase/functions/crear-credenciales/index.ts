...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: adminCheck, error: adminErr } = await supabaseUser.from('user_roles').select('role').eq('role', 'admin').maybeSingle();
    if (adminErr) {
    ...
    if (payload?.action === 'reset') {
      ...
      const { data: prof, error: profErr } = await supabaseAdmin.from('profesionales').select('user_id, correo').eq('id', profesionalId).maybeSingle();
      if (profErr) {
      ...
      if (!prof?.user_id) {
        return new Response(JSON.stringify({
          error: 'El profesional no tiene usuario vinculado'
        }), {
          status: 400,
          headers: {
            'Content-Type': 'application/json',
    ...
    if (createErr) {
      ...
      if (String(createErr.message || '').toLowerCase().includes('already') || createErr.status === 422) {
        const { data: existingProfile, error: profErr } = await supabaseAdmin.from('profiles').select('user_id').eq('correo_electronico', email).maybeSingle();
        if (profErr || !existingProfile?.user_id) {
          console.error('Usuario ya existía pero no se pudo recuperar user_id', profErr);
          return new Response(JSON.stringify({
            error: 'El usuario ya existe y no se pudo vincular automáticamente.'
          }), {
            status: 409,
            headers: {
              'Content-Type': 'application/json',
    ...
    const { error: linkErr } = await supabaseAdmin.from('profesionales').update({
      user_id: createdUserId
    }).eq('id', profesionalId);
    if (linkErr) {
      console.error('Error vinculando profesional:', linkErr);
      return new Response(JSON.stringify({
        error: 'No se pudo vincular el profesional'
      }), {
        status: 500,
        headers: {
          'Content-Type': 'application/json',
