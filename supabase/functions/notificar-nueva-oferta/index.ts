...
serve(async (req)=>{
  ...
  try {
    ...
    // Get offer details
    const { data: oferta, error: ofertaError } = await supabaseClient.from('ofertas').select(`
        *,
        autos_venta (
          *,
          clientes (*)
    ...
    const { data: ownerUser, error: ownerError } = await supabaseClient.auth.admin.getUserByEmail(oferta.autos_venta.clientes.correo_electronico);
    ...
    const { error: notificationError } = await supabaseClient.from('notificaciones').insert({
