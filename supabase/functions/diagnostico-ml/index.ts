...
serve(async (req) => {
  ...
  try {
    ...
    const resultados: any = {
      html_preview: html.substring(0, 2000), // Primeros 2000 caracteres
    ...
    const mlmLinks = [...html.matchAll(mlmRegex)];
    ...
    return new Response(
      JSON.stringify({
        ...
        recommendation: resultados.dynamic_content.likely_spa ? 
          'La página parece usar contenido dinámico. Necesitamos un enfoque diferente.' :
          'La página es estática. Los patrones pueden necesitar ajuste.'
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
