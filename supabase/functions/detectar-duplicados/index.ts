...
const priceSimilarity = (price1, price2)=>{
...
const mileageSimilarity = (km1, km2)=>{
  if (!km1 || !km2) return 0;
...
const calcularSimilitud = (anuncio1, anuncio2)=>{
  const similitudes = {};
  // 1. Similitud exacta por hash de contenido
  if (anuncio1.hash_contenido && anuncio2.hash_contenido && anuncio1.hash_contenido === anuncio2.hash_contenido) {
    return {
      score: 1.0,
      tipo: 'exacto',
      detalles: {
        razon: 'hash_contenido_identico'
      }
    };
  }
  ...
  if (anuncio1.titulo && anuncio2.titulo) {
    const tituloSim = Math.max(jaccardSimilarity(anuncio1.titulo, anuncio2.titulo), levenshteinSimilarity(anuncio1.titulo, anuncio2.titulo));
...
const guardarSimilitud = async (anuncio1Id, anuncio2Id, similitud)=>{
  try {
    const { error } = await supabase.from('anuncios_similares').upsert({
...
const detectarDuplicados = async (limite, umbralSimilitud = 0.7)=>{
  // Obtener anuncios activos ordenados por fecha de creación
  const { data: anuncios, error } = await supabase.from('anuncios_vehiculos').select('id, marca, modelo, ano, precio, kilometraje, titulo, descripcion, hash_contenido, created_at').eq('activo', true).order('created_at', {
    ascending: false
  }).limit(limite);
  if (error) {
    throw error;
