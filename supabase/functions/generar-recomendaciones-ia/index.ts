...
interface SolicitudRecomendaciones {
...
interface RespuestaRecomendaciones {
...
serve(async (req) => {
  ...
  try {
    ...
    const solicitud: SolicitudRecomendaciones = await req.json();
...
Considera el mercado mexicano, precios en pesos, y preferencias locales. Sé específico y práctico.`;
    ...
    const intentarOpenAI = async (intentos = 3): Promise<Response> => {
    ...
    let respuestaIA: RespuestaRecomendaciones;
    try {
    ...
    const recomendacionesValidas = respuestaIA.recomendaciones.filter(rec => 
      rec.tipo && rec.prioridad && rec.titulo && rec.descripcion && rec.accion
    );

    const respuestaFinal: RespuestaRecomendaciones = {
    ...
    const fallbackResponse: RespuestaRecomendaciones = {
      recomendaciones: [
        {
          tipo: 'mercado',
          prioridad: 'media',
    ...
    return new Response(JSON.stringify(fallbackResponse), {
      status: 200, // Devolver 200 para que el frontend maneje gracefully
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
});