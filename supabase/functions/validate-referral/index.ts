...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: codeData, error: codeError } = await supabase.from('referral_codes').select('user_id, uses_count, max_uses, active').eq('code', referral_code).eq('active', true).single();
    ...
    const { data: existingReferral } = await supabase.from('user_referrals').select('id').eq('referrer_id', codeData.user_id).eq('referee_id', user.id).single();
    if (existingReferral) {
      return new Response(JSON.stringify({
