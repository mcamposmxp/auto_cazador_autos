...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: vehiculosSimilares, error } = await supabaseClient.from('anuncios_vehiculos').select('precio, kilometraje, ano').ilike('marca', `%${marca}%`).ilike('modelo', `%${modelo}%`).gte('ano', ano - 2) // Rango de 2 años antes
    .lte('ano', ano + 2) // Rango de 2 años después
    .not('precio', 'is', null).gt('precio', 0).eq('activo', true).limit(50);
    if (error) {
...
function calcularPrecioBasePorMarca(marca, ano) {
  const marcasLujo = [
    'bmw',
    'mercedes',
    'audi',
