...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: anunciosCorregidos } = await supabase.from('anuncios_vehiculos').select('id, titulo, marca, modelo, ano, precio, precio_original, kilometraje, ubicacion, sitio_web').eq('sitio_web', 'mercadolibre.com.mx').order('created_at', {
      ascending: false
    });
    ...
    const ejemplosPrecios = anunciosCorregidos?.map((anuncio)=>({
        ...
        correcto: anuncio.precio_original === '425,900' ? anuncio.precio === 425900 : anuncio.precio_original === '329,000' ? anuncio.precio === 329000 : anuncio.precio_original === '298,000' ? anuncio.precio === 298000 : true
      })) || [];
