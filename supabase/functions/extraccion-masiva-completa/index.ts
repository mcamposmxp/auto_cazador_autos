...
async function crearSesionExtraccion(configuracion) {
  const sesionId = crypto.randomUUID();
  console.log(`🎯 Creando sesión de extracción: ${sesionId}`);
  // Crear estadísticas iniciales
  const { error: errorStats } = await supabase.from('estadisticas_extraccion').insert([
    {
      sesion_id: sesionId,
...
// Actualizar progreso
async function actualizarProgreso(sesionId, estrategia, parametro, datos) {
  const { error } = await supabase.from('progreso_extraccion').upsert([
    {
      sesion_id: sesionId,
      estrategia,
      parametro,
...
function construirUrlML(estrategia, parametro, pagina = 1) {
...
async function extraerUrlsDePagina(url, maxUrls = 200) {
...
async function procesarEstrategia(sesionId, estrategia, configuracion, timeoutTime) {
  const resultados = {
    estrategia,
  ...
  const elementos = ESTRATEGIAS_EXTRACCION[estrategia];
  ...
  const elementosLimitados = Array.isArray(elementos) ? elementos.slice(0, configuracion.max_elementos_por_estrategia || 10) : elementos;
...
async function extraccionMasivaCompleta(configuracion) {
  ...
  const estrategiasHabilitadas = configuracion.estrategias || [
    'categorias',
    'marcas',
    'estados'
  ];
  ...
  await supabase.from('estadisticas_extraccion').update({
...
serve(async (req)=>{
  ...
  try {
    ...
    const configCompleta = {
      ...
      max_elementos_por_estrategia: 15,
      max_paginas_por_elemento: 20,
      lote_size: 400,
      timeout_minutos: 240,
      estimado_urls: 100000,
      ...configuracion
    };
    ...
    const { data: estadisticasFinales } = await supabase.from('anuncios_vehiculos').select('id').eq('sitio_web', 'mercadolibre.com.mx');
