...
const normalizarMarca = async (marcaOriginal)=>{
  ...
  if (mejorCoincidencia.distancia <= 2) {
    ...
    return {
      marca_normalizada: mejorCoincidencia.marca,
      confianza
    };
  }
  // Si no encontramos coincidencia, devolvemos la marca original
  return {
    marca_normalizada: marcaLower,
    confianza: 0.5
  };
};
...
const normalizarPrecio = (precioOriginal)=>{
...
const normalizarKilometraje = (kilometrajeOriginal)=>{
...
const normalizarAno = (anoOriginal)=>{
...
const normalizarAnuncio = async (anuncio)=>{
  try {
    ...
    const { error: updateError } = await supabase.from('anuncios_vehiculos').update(datosNormalizados).eq('id', anuncio.id);
    if (updateError) {
      throw updateError;
    }
    ...
    if (marcaNormalizada.marca_normalizada && marcaNormalizada.confianza > 0.7) {
      const { error: marcaError } = await supabase.from('marcas_normalizadas').upsert({
    ...
    // Marcar como error
    await supabase.from('anuncios_vehiculos').update({
...
serve(async (req)=>{
  ...
  try {
    ...
    let query = supabase.from('anuncios_vehiculos').select('id, titulo, marca, modelo, ano, precio, precio_original, kilometraje, ubicacion, sitio_web, datos_raw').eq('estado_normalizacion', 'pendiente').eq('activo', true).limit(limite);
    if (sitio_web) {
