...
const getRandomUserAgent = (customAgents)=>{
...
function extractData(html, selectores) {
  const data = {
    titulo: '',
    precio: null,
    precio_original: '',
  ...
  try {
    ...
    for (const pattern of tituloPatterns){
      ...
      if (match && match[1]?.trim() && match[1].trim().length > 5) {
        data.titulo = match[1].trim().replace(/&nbsp;/g, ' ').replace(/\s+/g, ' ').replace(/&amp;/g, '&').replace(/&quot;/g, '"');
    ...
    for (const pattern of anoPatterns){
      if (pattern.flags?.includes('g')) {
        const matches = [
          ...html.matchAll(new RegExp(pattern.source, 'gi'))
        ];
    ...
    if (anosEncontrados.length > 0) {
      ...
      data.ano = Object.keys(frecuencias).map(Number).sort((a, b)=>frecuencias[b] - frecuencias[a] || b - a)[0];
    ...
    if (data.titulo) {
      ...
      if (marcaEncontrada) {
        data.marca = marcaEncontrada.split(' ').map((p)=>p.charAt(0).toUpperCase() + p.slice(1)).join(' ');
    ...
    for (const pattern of ubicacionPatterns){
      ...
      if (match && match[1]?.trim() && match[1].trim().length > 2) {
        const ubicacion = match[1].trim().replace(/&nbsp;/g, ' ').replace(/\s+/g, ' ');
    ...
    if (imagenesMatches) {
      const imagenesUnicas = Array.from(new Set(imagenesMatches)).filter((url)=>{
        const urlLower = url.toLowerCase();
        return !urlLower.includes('logo') && !urlLower.includes('icon') && !urlLower.includes('favicon') && !urlLower.includes('sprite') && !urlLower.includes('banner') && (urlLower.includes('mlstatic') || urlLower.includes('mlibre')); // Solo imágenes de MercadoLibre
      }).slice(0, 10);
...
const registrarLog = async (log)=>{
  try {
    const { error } = await supabase.from('logs_extraccion').insert(log);
    if (error) {
...
const extraerAnuncio = async (url, config)=>{
  ...
  try {
    ...
    const { data: existente } = await supabase.from('anuncios_vehiculos').select('id').eq('url_anuncio', url).single();
    if (existente) {
      // Actualizar anuncio existente
      const { error } = await supabase.from('anuncios_vehiculos').update({
        ...anuncioData,
      ...
      const { error } = await supabase.from('anuncios_vehiculos').insert(anuncioData);
      if (error) throw error;
    }
  ...
  try {
    ...
    if (!sitio_web || !urls || !Array.isArray(urls)) {
      return new Response(JSON.stringify({
        error: 'sitio_web y urls son requeridos'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: config, error: configError } = await supabase.from('configuracion_extraccion').select('*').eq('sitio_web', sitio_web).eq('activo', true).single();
    if (configError || !config) {
      return new Response(JSON.stringify({
        error: `No se encontró configuración para ${sitio_web}`
      }), {
        status: 404,
        headers: {
          ...corsHeaders,
    ...
    await supabase.from('configuracion_extraccion').update({
