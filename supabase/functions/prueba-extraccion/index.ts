...
serve(async (req)=>{
  ...
  try {
    ...
    const { count: totalAnuncios, error: errorTotal } = await supabase.from('anuncios_vehiculos').select('*', {
      count: 'exact',
      head: true
    });
    console.log('Total anuncios:', totalAnuncios, 'Error:', errorTotal);
    const { count: anunciosML, error: errorML } = await supabase.from('anuncios_vehiculos').select('*', {
      count: 'exact',
      head: true
    }).eq('sitio_web', 'mercadolibre.com.mx');
    console.log('Anuncios ML:', anunciosML, 'Error:', errorML);
    const { data: ultimosAnuncios, error: errorUltimos } = await supabase.from('anuncios_vehiculos').select('id, titulo, marca, modelo, ano, precio, ubicacion, fecha_extraccion').eq('sitio_web', 'mercadolibre.com.mx').order('created_at', {
      ascending: false
    }).limit(5);
