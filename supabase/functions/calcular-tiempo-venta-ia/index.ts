...
serve(async (req) => {
  ...
  try {
    ...
    const requestData: CalculoTiempoRequest = await req.json();
    ...
    if (isAuthenticated && userId) {
      ...
      if (error || !data) {
        ...
        return new Response(
          JSON.stringify(calculateFallbackTime(requestData)),
          {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            status: 200,
          }
    ...
    const esTemporadaAlta = [11, 12, 1].includes(new Date().getMonth() + 1); // Nov-Ene
...
- Época del año (considera si es temporada alta o baja para autos)`;
    ...
    // Parse AI response
    let resultado: CalculoTiempoResponse;
    try {
    ...
    resultado.velocidadVenta = ['rapida', 'moderada', 'lenta'].includes(resultado.velocidadVenta) 
...
function calculateFallbackTime(requestData: CalculoTiempoRequest): CalculoTiempoResponse {
  const diferenciaPorcentual = ((requestData.precioSeleccionado - requestData.precioRecomendado) / requestData.precioRecomendado) * 100;
  ...
  let velocidadVenta: 'rapida' | 'moderada' | 'lenta';
  if (tiempoBase <= 20) {
