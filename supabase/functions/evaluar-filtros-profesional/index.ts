  ...
  try {
    ...
    if (!marca || !modelo || !ano || kilometraje === undefined) {
      return new Response(JSON.stringify({
        error: 'Marca, modelo, año y kilometraje son requeridos'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    if (!precio_estimado) {
      ...
      try {
        const { data: precioData, error: precioError } = await supabaseClient.functions.invoke('calcular-precio-estimado-vehiculo', {
          body: {
            marca,
            modelo,
            ano,
    ...
    const { data: profesionales, error: profesionalesError } = await supabaseClient.from('profesionales').select('id, negocio_nombre').eq('activo', true);
    if (profesionalesError) {
      console.error('Error obteniendo profesionales:', profesionalesError);
      throw profesionalesError;
    }
    const profesionalesCoincidentes = [];
    const detallesCoincidencias = [];
    // Evaluar filtros para cada profesional
    for (const profesional of profesionales){
      try {
        // Llamar a la función de base de datos para evaluar filtros
        const { data: cumpleFiltros, error: filtrosError } = await supabaseClient.rpc('evaluar_filtros_vehiculo', {
        ...
        if (cumpleFiltros) {
          ...
          const { data: filtros } = await supabaseClient.from('profesional_filtros_ofertas').select('filtros_vehiculo, tipo_filtro').eq('profesional_id', profesional.id).eq('activo', true).single();
          ...
          if (filtros && filtros.tipo_filtro === 'personalizado') {
            ...
            if (filtrosVehiculo.marcas_modelos && filtrosVehiculo.marcas_modelos.length > 0) {
              ...
              if (precioEstimadoFinal < filtrosVehiculo.rango_precio.min || precioEstimadoFinal > filtrosVehiculo.rango_precio.max) {
                motivoRechazo = 'Precio fuera de rango';
              }
            } else if (filtrosVehiculo.rango_kilometraje?.activo) {
              if (kilometraje < filtrosVehiculo.rango_kilometraje.min || kilometraje > filtrosVehiculo.rango_kilometraje.max) {
