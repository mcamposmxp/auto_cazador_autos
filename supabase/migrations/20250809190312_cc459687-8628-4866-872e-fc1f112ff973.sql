-- Ensure two professional profiles exist for sample offers
WITH ins1 AS (
  ...
  SELECT gen_random_uuid(), 'Agencia', 'Uno', '5551112222', 'agencia1@example.com', 'profesional',
  ...
  SELECT gen_random_uuid(), 'Agencia', 'Dos', '5552223333', 'agencia2@example.com', 'profesional',
...
-- Add additional RLS policies using JWT email claim (keeps existing policies intact)
CREATE POLICY IF NOT EXISTS "Owners view offers via jwt email"
ON public.ofertas
FOR SELECT
USING (
...
CREATE POLICY IF NOT EXISTS "Owners update offers via jwt email"
ON public.ofertas
FOR UPDATE
USING (
  (profesional_id = auth.uid())
